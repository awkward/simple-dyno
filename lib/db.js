...
export function setConfig(options = {accessKeyId: "", secretAccessKey: "", region: "eu-west-1", runLocal: true, inMemory: true}) {
  if((debug.local || process.env.NODE_ENV === 'test') && options.runLocal) {
    try {
      let stdout = execSync("killall java");
      logger(`killall java: ${stdout.split('\n').join('')}`);
    } catch (e) {}
    let dbDir = null;
    if(!options.inMemory) dbDir = "/database";
  ...
  } else if(debug.local || process.env.NODE_ENV === 'test') {
